...
void InitCommandLine(int argc, char** argv, po::variables_map* conf) {
	...
	("intersection_strategy,I",po::value<string>()->default_value("cube_pruning"), "Intersection strategy for incorporating finite-state features; values include Cube_pruning, Full, Fast_cube_pruning, Fast_cube_pruning_2, Guided_pruning")
...
int main(int argc, char** argv) {
	...
	if (LowercaseString(conf["intersection_strategy"].as<string>()) == "guided_pruning") {
		palg = 4;
		cerr << "Using Guided Pruning.\n";
	}
			...
			while(*in) {
				...
				if (conf.count("show_tree_structure"))
					...
					if (has_late_models) {
						...
						prob_t curr = ViterbiESentence(forest, &trans); //AS
						sum *= curr; //AS
						cerr << "BEST PATH: " << curr << endl;
						cerr << "best path log: "<<log(curr)<<endl;
						tot_count++;//AS 
						cerr << "Average score of translation sentence (log): " << log(sum)<<"/"<<tot_count<<" = "<< log(sum)/tot_count<< endl;//AS
